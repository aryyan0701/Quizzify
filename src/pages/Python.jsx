        ...
        {currentQuestionIndex < pythonQuestions.length ? (
          <>
            <p className={AnswerCounter}>
              Correct Answers: <span data-testid="quiz-answer-counter">{correctAnswers}</span> /{' '}
              {pythonQuestions.length}
            </p>

            <div className={QuestionContainer}>
              <p>Question: {currentQuestionIndex + 1}</p>
              <ShowpythonQue {...pythonQuestions[currentQuestionIndex]} answerQuestion={answerQuestion} />
            </div>
          </>
        ) : (
          <div className={FinalScoreContainer}>
            ...
            <p className={FinalScoreCounter}>
              <span data-testid="final-answer-counter">{correctAnswers}</span> / {pythonQuestions.length}
            </p>
          </div>
        )}
      </div>
